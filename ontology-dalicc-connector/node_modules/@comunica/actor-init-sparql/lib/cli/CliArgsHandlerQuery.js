...
class CliArgsHandlerQuery {
    ...
    async handleArgs(args, context) {
        ...
        context[context_entries_1.KeysInitSparql.queryFormat] = args.inputType;
        // Define the datetime
        if (args.dateTime) {
            context[context_entries_1.KeysHttpMemento.datetime] = new Date(args.dateTime);
        }
        // Set the proxy
        if (args.proxy) {
            context[context_entries_1.KeysHttpProxy.httpProxyHandler] = new actor_http_proxy_1.ProxyHandlerStatic(args.proxy);
        }
    }
}
