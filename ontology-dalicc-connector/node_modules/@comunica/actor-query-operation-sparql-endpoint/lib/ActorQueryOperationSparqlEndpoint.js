...
class ActorQueryOperationSparqlEndpoint extends bus_query_operation_1.ActorQueryOperation {
    ...
    async test(action) {
        ...
        const destination = await utils_datasource_1.DataSourceUtils.getSingleDestination(action.context);
        const sourceType = source ? bus_rdf_resolve_quad_pattern_1.getDataSourceType(source) : undefined;
        const destinationType = destination ? bus_rdf_update_quads_1.getDataDestinationType(destination) : undefined;
        const sourceValue = source ? bus_rdf_resolve_quad_pattern_1.getDataSourceValue(source) : undefined;
        const destinationValue = destination ? bus_rdf_update_quads_1.getDataDestinationValue(destination) : undefined;
